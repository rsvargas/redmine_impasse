class ImpasseTestCaseController < ImpasseAbstractController
  ...
  private
  def new_node
    @node = Impasse::Node.new(params.require(:node).permit!)

    case params[:node_type]
    when 'test_case'
      @test_case = Impasse::TestCase.new(params.require(:test_case).permit!)
      @test_case.active = true
      @test_case.importance = 2
      @node.node_type_id = 3
    else
      @test_case = Impasse::TestSuite.new(params.require(:test_case).permit!)
      @node.node_type_id = 2
    end
  end
