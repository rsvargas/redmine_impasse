...
jQuery(document).ready(function ($) {
        ...
        $.ajax({
            ...
            success: function (html) {
                ...
                $.getJSON(IMPASSE.url.testKeywords, function (json) {
                    setupKeyword($("input[name=node_keywords]", dialog[node_type]), json)
                });
    ...
    $("#values_fixed_version_id").on("change", function (e) {
        ...
        $.ajax({
            url: IMPASSE.url.requirementIssues,
            data: {
                "fields": ['fixed_version_id'],
                "values[fixed_version_id]": [version_id],
                "operators[fixed_version_id]": version_id ? "=" : "!*"
            },
            success: function (html) {
                $("#requirements-view").html(html).show();
            },
